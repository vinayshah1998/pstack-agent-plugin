...
Before dispatching investigators, enumerate the MCP tools and other evidence integrations exposed by the active host. On Kiro, use the active agent's available MCP tool list and permissions; do not inspect client-private configuration files.
...
Dispatch each investigator through the host's subagent capability. Use the configured `code exploration` role when available and `auto` otherwise. Configure actual tools and permissions to retain the read-only MCP access required for its evidence category while withholding file-write and external-write tools. Do not assume a host's prose `readonly` or Ask-mode setting retains MCP access; investigators still must not write anything.
...
Dispatch one synthesis agent through the host's subagent capability. Use the configured `explanation and synthesis` role when available and `auto` otherwise. Give it read-only access to the evidence tools needed to spot-check citations and no file-write tools.
