...
Dispatch each investigator through the host's subagent capability. Use the configured `code exploration` role when available and `auto` otherwise. Enforce no-write behavior with the target agent's tools and permissions while retaining read-only MCP access required for its evidence category.
