...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured `performance investigation` role from `setup-pstack`; review the diff. Capture a post-fix trace.
